    ...
    float *work )
{
    ...
    ( MagmaUpperStr, MagmaNonUnitStr, &ib, work, &ib, &info );
}
...
/***************************************************************************//**
    ...
    @param[out]
    tau     float PRECISION array, dimension (min(M,N))
            The scalar factors of the elementary reflectors (see Further
            Details).

    @param[out]
    dT      (workspace) float PRECISION array on the GPU,
    ...
    magma_int_t *info )
{
    ...
    *info = 0;
    ...
    int pd_desired_freq;
    int pd_freq;
    int pd_curr_freq = 1000;
    int pd_base_freq = 1000;
    ...
    float * chk_vt;
    int ld_chk_vt = 2;
    magma_smalloc_pinned(&chk_vt, 2 * nb * sizeof(float));
    for (int i = 0; i < nb; ++i) {
    ...
    //printf("%d\n", nb);
    float * dev_chk_vt;
    size_t pitch_dev_chk_vt = magma_roundup(2 * sizeof(float), 32);
    int ld_dev_chk_vt;
    
    magma_smalloc(&dev_chk_vt, pitch_dev_chk_vt * nb);
    ld_dev_chk_vt = pitch_dev_chk_vt / sizeof(float);
    magma_sgetmatrix(2, nb,
                     chk_vt, ld_chk_vt, 
    ...
    float * dT_colchk;
    size_t pitch_dT_colchk = magma_roundup((gpu_row / nb) * 2 * sizeof(float), 32);
    int lddt_colchk = pitch_dT_colchk / sizeof(float);
    if (COL_FT || ROW_FT) magma_smalloc(&dT_colchk, pitch_dT_colchk * gpu_col);
    if (COL_FT || ROW_FT) cudaMemset(dT_colchk, 0, pitch_dT_colchk * gpu_col);

    float * dT_colchk_r;
    size_t pitch_dT_colchk_r = magma_roundup((gpu_row / nb) * 2 * sizeof(float), 32);
    int lddt_colchk_r = pitch_dT_colchk_r / sizeof(float);
    if (COL_FT || ROW_FT) magma_smalloc(&dT_colchk_r, pitch_dT_colchk_r * gpu_col);

    float * dT_rowchk;
    size_t pitch_dT_rowchk = magma_roundup(gpu_row * sizeof(float), 32);
    int lddt_rowchk = pitch_dT_rowchk / sizeof(float);
    if (COL_FT || ROW_FT) magma_smalloc(&dT_rowchk, pitch_dT_rowchk * (gpu_col / nb) * 2);
    if (COL_FT || ROW_FT) cudaMemset(dT_rowchk, 0, pitch_dT_rowchk * (gpu_col / nb) * 2);


    float * dT_rowchk_r;
    size_t pitch_dT_rowchk_r = magma_roundup(gpu_row * sizeof(float), 32);
    int lddt_rowchk_r = pitch_dT_rowchk_r / sizeof(float);
    if (COL_FT || ROW_FT) magma_smalloc(&dT_rowchk_r, pitch_dT_rowchk_r * (gpu_col / nb) * 2);
    ...
    if ( nb > 1 && nb < minmn ) {
        ...
        for (i = 0; i < minmn-nb; i += nb) {
            ...
            if (predict) {
                ...
                pd_time_ratio = (2*((float)m-i)*ib*ib-2*ib*ib*ib/3)/(2*((float)m-last_prof_iter*ib)*ib*ib-2*ib*ib*ib/3);
                dt_time_ratio = (((double)m-i)*nb)/(((double)m-nb*last_prof_iter)*nb);
                tmu_time_ratio = (float)abft_slarfb_flops(MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
                                                           m-(i-ib), n-(i-ib)-2*ib, ib, ib, COL_FT, ROW_FT, CHECK_BEFORE, CHECK_AFTER)/
                                        abft_slarfb_flops(MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
            ...
            if (i > 0) {
                ...
                abft_slarfb_gpu( MagmaLeft, MagmaConjTrans, MagmaForward, MagmaColumnwise,
                                 m-old_i, cols, old_ib,
                                  dA(old_i, old_i         ), ldda, dT(old_i), nb,
            ...
            //     float * correct_result = new float[rows*ib];
            ...
            //     int R = 100;
            //     float ** tmp = new float*[R];
            //     for (int r = 0; r < R; r++) tmp[r] = new float[rows*ib];
            ...
            //         pid = start_measure_cpu();
            //         for (int r = 0; r < R; r++) {
            //             lapackf77_sgeqrf( &rows, &ib, tmp[r],    &rows, &tau[i], hwork, &lhwork, info );
            //             // Form the triangular factor of the block reflector in hwork
            //             // H = H(i) H(i+1) . . . H(i+ib-1)
            //             lapackf77_slarft( MagmaForwardStr, MagmaColumnwiseStr,
            //                               &rows, &ib,
            ...
            // printf("%d, pd-tmu, %f, %f, %f, %f, %f, %f, %d, %d, \n", i, pd_time_actual, dt_time_actual, tmu_time_actual, pd_time_pred, dt_time_pred, tmu_time_pred, pd_freq, tmu_freq);
