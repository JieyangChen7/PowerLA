        ...
        // for( j=0; j < 1536; j += nb ) {
            ...
            if (predict) {
                ...
                if (reclaim_slack) {
                    ...
                    if (pd_time_pred * pd_freq_ratio + cpu_adj_time + dt_time_pred <= max_pd_tmu) {
            ...
            if ( j > 0 ) {
                ...
                if (reclaim_tmu && j > nb) {
                    if (tmu_freq != tmu_curr_freq) {
                        ...
                        // double tt = duration_cast<milliseconds>(t2 - t1).count();
                ...
                abft_dgemm( MagmaNoTrans, MagmaNoTrans,
                            ...
                            dAT_rowchk_r(j, j+nb), lddat_rowchk_r,
                            dev_chk_v, ld_dev_chk_v, 
                            COL_FT, ROW_FT, DEBUG, true, false,
                            queues[1], queues[1]);
